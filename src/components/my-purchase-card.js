...
class MyPurchaseCard extends Component {
  constructor(props) {
    ...
    this.state = { publication: {}, loading: true }
  ...
  async loadPublication(addr) {
    try {
      ...
      this.setState({ publication, loading: false })
    } catch(error) {
      console.error(`Error fetching contract or IPFS info for publication: ${addr}`)
    }
  }

  componentDidMount() {
    this.loadPublication(this.props.purchase.publicationAddress)
  ...
  render() {
    ...
    const { category, name, pictures, price } = translatePublicationCategory(this.state.publication)
