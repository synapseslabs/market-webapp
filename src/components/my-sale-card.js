...
class MySaleCard extends Component {
  ...
  render() {
    const { publication, purchase, user } = this.props

    if (!publication) {
      console.error(`Publication not found for purchase ${purchase.address}`)
      return null
    }

    const { name, pictures } = publication
    ...
    const price = `${Number(publication.price).toLocaleString(undefined, {minimumFractionDigits: 3})} ${this.props.intl.formatMessage(this.intlMessages.ETH)}` // change to priceEth
